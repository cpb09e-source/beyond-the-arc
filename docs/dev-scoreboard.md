...
/game?id=214837&date=2026-02-07      one LIVE game; both parameters required
/games/2026/214837-duke-vs-north-carolina/   any archived game, by its real URL
/scoreboard/2026-02-07/              any archived day, by its real URL
```
...
## The archive: last season is static files, not the function

Every completed season is baked into `public/data/scoreboard/<season>/` (one
slate per day) and `public/data/games/<season>/` (one bundle per game) by:

```
npx tsx scripts/build-scoreboard-archive.mts            # 2026, ~93s, no API calls
npx tsx scripts/build-scoreboard-archive.mts --fetch-lines   # once per season
```

It works by importing the two Netlify handlers and replacing `fetch` with a
shim that answers `api.collegebasketballdata.com` out of `data/cbbd/<season>/`.
The output is therefore the exact shape the live path returns — switching a
date between the archive and the function cannot change the render.

Those days have real pages: `/scoreboard/2026-02-07/` and
`/games/2026/214837-duke-vs-north-carolina/`. Both prerender their scores, so
...
Without that, every game page 404s on its box score in production — the local
files are stripped from `out/` by design.

## Out of season / demo mode

`SCOREBOARD_MODE` in `src/lib/flags.ts` is `"demo"`, which since the archive
landed means only "no season is being played". The ticker and `/scoreboard`
open on the last night of the last archived season, read from static files.
There are no baked sample files any more.

**To go live:** set `SCOREBOARD_MODE` to `"live"` and `DEMO_DATE` to `null` in
...
deployed function. Against an archived day the same transform runs on the
client instead, gated on
