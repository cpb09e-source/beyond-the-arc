/**
 * Coach data layer, PURE HALF — every shape, the joins, the composite score
 * and the bracket lookups, with no file reads anywhere in it.
 *
 * SPLIT OUT OF lib/coaches.ts SO SOMETHING WITHOUT A FILESYSTEM CAN BUILD A
 * COACH. That module reads src/data and public/data off disk through node:fs,
 * so nothing that runs in a browser could import any part of it — not a client
 * component (coach-style-panel.tsx was reaching through it for
 * STYLE_DIMENSIONS), and not the desktop app's renderer. What stayed there is
 * the file reading; everything it computes is here, and it hands the parsed
 * files to buildCoachProfiles() exactly as a renderer holding the same JSON
 * would.
 *
 * IMPORTS ARE HELD TO MODULES WITH NO IMPORTS OF THEIR OWN (coach-slug,
 * conf-tiers): no node:*, no next/*, no React, no `process`. Keep it that way,
 * or the renderer that imports this breaks in a way that names the wrong file.
 *
 * Joins four sources, all plain JSON (see CoachSources for the exact shapes):
 ...
 *   3. team-season rows (teams-all.json, window-filtered) — conference, record,
 *      ratings and play style per (team, year)
 *   4. src/data/tournament-games.json — the bracket, for tournament records and
 *      the composite's upset bonus
 *
 * Output shapes:
 *   - CoachIndexRow: one row per unique coach (for /coaches index)
 *   - CoachProfile:  full timeline + per-school breakdown (for /coaches/<slug>)
 *
 * If coach-history is empty (scraper still running), the layer gracefully
 * degrades to ESPN-only single-season data.
 */
...
/** A Bart team name as this layer spells it (the one rename is USC). */
 ...
 * without pulling lib/coaches.ts's file-reading loaders into the browser bundle.
...
export function styleFromSeasonStats(ss: Record<string, number | null> | null | undefined): CoachStyle | null {
  if (!ss) return null;
  return {
 ...
 * reason this exists.
 */
export function styleLeagueAverages(teams: ReadonlyArray<CoachSourceTeamRow>): Map<number, CoachStyle> {
...
/** coach-history.json — { [Bart team name]: { ["2013".."2026"]: SrSeason } }. */
export type CoachHistory = Record<string, Record<string, SrSeason>>;

/** team-coaches.json — { [Bart team name]: EspnCoach }, the 2025-26 ESPN snapshot. */
export type EspnCoachSnapshot = Record<string, EspnCoach>;

/** tournament-games.json — { ["2013".."2026"]: TourneyGame[] }. No 2020. */
export type TournamentGamesByYear = Record<string, TourneyGame[]>;

/**
 * The fields of one team-season row that this layer reads.
 *
 * teams-all.json rows — StaticTeamSeasonRow — carry a great deal more, and are
 * accepted as they are; so is anything else carrying these fields.
 * `team_season_stats` is read for pace, fg3a_rate, fta_rate, orb_pct, tov_pct,
 * ast_pct, efg_pct_def, tov_pct_def and orb_pct_def only.
 */
export type CoachSourceTeamRow = {
  name: string;
  year: number;
  conference?: string | null;
  bta_rtg?: number | null;
  ...
  team_season_stats?: { readonly [stat: string]: number | null | undefined } | null;
};

/**
 * Everything buildCoachProfiles needs: the four source documents, parsed.
 *
 * `teams` is the row set lib/coaches.ts passes — readAllTeams(), which is
 * teams-all.json filtered to the site's season window (isUsableSeason, today
 * 2014-2027). public/data/teams-by-year/<year>.json concatenated over the same
 * years is the same rows, in the same order within each year, and builds
 * identical profiles.
 *
 * THE WINDOW IS NOT OPTIONAL. The history starts in 2013 and the window does
 * not, so every 2013 season is built with no conference and no ratings. Hand
 * this the 2013 rows as well and those seasons gain both — and the composite,
 * Adj Net average, best BTA rank and current conference of 342 coaches move
 * with them. Rows before 2013 change nothing. Missing files are empty
 * objects/arrays, which is how lib/coaches.ts treats them.
 */
export type CoachSources = {
  history: CoachHistory;
  espn: EspnCoachSnapshot;
  teams: ReadonlyArray<CoachSourceTeamRow>;
  tournamentGames: TournamentGamesByYear;
};

/** Current-season conference and record for a team, from its LATEST_YEAR row. */
export type CoachTeamMeta = {
  conference: string | null;
  record: string | null;
  wins: number | null;
  losses: number | null;
...
/** The (team, year) joins built from the team rows. Team names are post-overrideTeam. */
export type CoachRatingsIndex = {
  meta: Map<string, CoachTeamMeta>;                    // team → LATEST_YEAR conference + record
...
type RawSourceData = CoachRatingsIndex & {
  history: CoachHistory;                               // { team: { year: SrSeason } }
  espn: EspnCoachSnapshot;                             // { team: EspnCoach }
};

/**
 * Index the team rows by (team, year): conference, the four ratings with their
 * within-year percentiles and BTA rank, and the season's play style — plus the
 * current-season record per team for coaches the history has not reached.
 */
export function buildRatingsIndex(teams: ReadonlyArray<CoachSourceTeamRow>): CoachRatingsIndex {
  ...
  return { meta, confByTeamYear, ratingsByTeamYear };
}
...
/**
 * Every coach's full profile, from the parsed source documents.
 *
 * The whole pipeline in one call, and the only entry point a caller needs.
 * lib/coaches.ts reads the four files and calls this; anything that can get
 * the same JSON some other way calls it directly and gets the same profiles,
 * in the same order.
 */
export function buildCoachProfiles(sources: CoachSources): CoachProfile[] {
  const { history, espn, teams, tournamentGames } = sources;
  const { meta, confByTeamYear, ratingsByTeamYear } = buildRatingsIndex(teams);
  const raw: RawSourceData = { history, espn, meta, confByTeamYear, ratingsByTeamYear };
  ...
  // Attach composite résumé score. Build the (team, year) bracket lookup once
  // and share it across all profiles.
...
/**
 * A profile as an index row: the same object minus its heavy per-season
 * fields, which is what the /coaches page ships.
 */
export function toIndexRow(profile: CoachProfile): CoachIndexRow {
  // Strip the heavy fields for the index page. composite_score is small (one
  // number) and serves the table, so it stays.
  const { by_year: _b, schools: _s, best_season: _bs, worst_season: _ws, best_record_season: _br, ...row } = profile;
