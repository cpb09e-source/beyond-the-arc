import type { Basis, Cell, SplitSeason } from "@/components/players/player-splits";

/**
 * The Player Overview's cards: which stats, in which views, and how each one is
 * read out of a split.
 *
 * MOVED HERE VERBATIM from src/components/players/player-stats-grid.tsx, so the
 * desktop app draws exactly the cards the site draws. The site's panel imports
 * them from here.
 */

export type Ctx = { season: SplitSeason | null; split: string; basis: Basis };

/** A stat's home block, which decides whether the basis toggle touches it. */
export type Block = "m" | "g" | "f" | "impact";
...
export type View = "overview" | "scoring" | "creation" | "defense";
export type Fmt = "int" | "num1" | "num2" | "pct1" | "signed1";
 ...
 */
export const VIEW_OPTIONS: Array<{ key: View | "everything"; label: string }> = [
...
export const CARDS: Array<{ title: string; views: View[]; stats: Def[] }> = [
  {
    title: "Impact",
    views: ["overview"],
    stats: [
...
export function fmt(v: number | null, f: Fmt): string {
...
export function read(ctx: Ctx, d: Def): Cell | null {
  if (!ctx.season) return null;
  if (d.derive) {
