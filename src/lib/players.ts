...
export const PLAYER_STAT_COLUMNS: PlayerStatColumn[] = [
  ...
  // it fits are luck-adjusted, the box prior underneath it runs on expected
  // threes and reads CBBD's per-game archive for opponent context and shot
  // detail, the ridge penalty is tuned against held-out games and split-half
  // reliability rather than set by hand, and the prior is calibrated against a
  // prior-free fit so the pipeline cannot feed itself.
  //
  ...
  { key: "ewins",  label: "eWins",  desc: "Estimated wins added vs an average player — the value form of EPM, converting per-possession impact into wins over the playing time a player actually got. Rewards doing it for 34 minutes a night rather than 18. Estimates for very low-usage players are eased toward average, because a player who ends few possessions leaves less evidence behind him either way.", group: "impact", format: "num2", field: "ewins" },
