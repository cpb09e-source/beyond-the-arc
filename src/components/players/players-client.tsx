...
import {
  playerEntity, type ExportInput, type MultiExportInput,
...
import { SORT_FIELD, exportColsFor, fmtGrid, packLookups, pinnedGridCol, viewGrid, type GridCol } from "@/lib/player-explorer-columns";
...
export function PlayersClient({ confsByYear }: { confsByYear: Record<string, string[]> }) {
  ...
  const dynamicCols: GridCol[] = useMemo(() => {
    ...
    const add = (key: string, allowDuplicate: boolean) => {
      // Either catalog, and never a filter-only stat: see pinnedGridCol.
      const col = pinnedGridCol(key);
      if (!col) return;
      const id = (col.field ?? col.packKey) as string;
      ...
      // Deduped against the ACTIVE view rather than a fixed default set.
      if (!allowDuplicate && viewFields.has(id)) return;
      seen.add(id);
      out.push(col);
    };
    ...
    <>
      ...
      <div id="players-leaderboard" className="bg-card border border-ink/10 border-x-0 lg:border-x rounded-none lg:rounded-xl shadow-md overflow-hidden ring-0 lg:ring-1 ring-ink/5 mt-6 max-md:mt-2 scroll-mt-6 -mx-6 lg:mx-0">
        ...
        <div
          ...
          className="overflow-auto overscroll-x-none players-scroll cursor-grab md:max-h-[calc(100vh-1.5rem)]"
          {...panHandlers}
        >
                      ...
                      <span className="flex items-center gap-2 sm:gap-2.5 min-w-0 sm:min-w-44">
                        ...
                        <span className="min-w-0">
                          ...
                          <span className="flex items-center gap-1.5 text-[0.66rem] text-ink-muted whitespace-nowrap leading-tight">
                            ...
                            {/* No rank badge on a phone. It was moved down here
                                when the name line ran out of room, but a column
                                of orange #1..#50 running down the meta line read
                                as the page's loudest element while saying only
                                what the RK column already says. Desktop keeps it
                                beside the name. */}
                          </span>
                        </span>
        ...
        {/* Desktop only. Below md the box has no vertical overflow to drive
            (see the cap note above), and this renders its track and both
            arrows unconditionally — a dead grey rail with buttons that do
            nothing. sync() nulls the THUMB when there is nothing to scroll,
            which is not the same as hiding the control. */}
