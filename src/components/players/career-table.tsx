      ...
      <div className="bg-card border border-hairline rounded-lg overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="border-b border-hairline text-left">
            <tr>
              <Th>Season</Th><Th>Team</Th><Th>Cl</Th>
              <Th align="right">GP</Th>
              <Th align="right">PPG</Th><Th align="right">RPG</Th><Th align="right">APG</Th>
              <Th align="right">3P%</Th><Th align="right">2P%</Th><Th align="right">FT%</Th>
            </tr>
          </thead>
          <tbody>
            {seasons.map((s) => (
              <tr key={s.year} className="border-b border-hairline/60 hover:bg-paper-deep/50 transition-colors">
                ...
                <Td align="right" className="tabular">{fmtNum(fromEnd(s.raw_row, 6), 1)}</Td>
                <Td align="right" className="tabular">{fmtPct(pctFromIdx(s.raw_row, 21))}</Td>
                <Td align="right" className="tabular">{fmtPct(pctFromIdx(s.raw_row, 18))}</Td>
                <Td align="right" className="tabular">{fmtPct(pctFromIdx(s.raw_row, 15))}</Td>
              </tr>
            ))}
          </tbody>
        </table>
