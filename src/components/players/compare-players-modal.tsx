...
export function ComparePlayersModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function buildShareRows(): ShareRow[] {
    return ROWS.map((row, ri) => {
      ...
      return {
        section: row.section && row.section !== ROWS[ri - 1]?.section ? row.section : undefined,
        label: row.label,
        cells: filledSlots.map((s) => {
          const raw = row.value(s.season, s.ctx);
          return {
            display: row.format ? row.format(raw) : raw == null ? "—" : String(raw),
            pct: row.percentileKey && s.season
              ? s.season.stats[row.percentileKey]?.percentile ?? null
              : null,
            best: bestKey === s.key,
            worst: worstKey === s.key,
          };
        }),
      };
    });
      ...
      >
        <div className="flex items-end justify-between gap-3 px-4 md:px-6 py-3 md:py-5 border-b border-hairline bg-paper-deep/30 shrink-0">
          <div>
            ...
            <h2 className="font-display text-xl md:text-3xl text-ink leading-none tracking-tight">Compare players</h2>
        ...
        <div ref={scrollerRef} className="max-md:flex-1 max-md:min-h-0 md:min-h-[60vh] md:max-h-[65vh] overflow-y-auto overscroll-contain">
          ...
          <>
          {/* ================= PHONE =================
              See the teams modal: the desktop table spends 224px of a 390px
              screen on the category column, so the label moves above the row
              and the width goes to the numbers. Players are named once in a
              sticky strip; surname only, because four full names do not fit
              and the logo already says which team. */}
          <div className="md:hidden">
            ...
            >
              {filledSlots.map((s) => (
                <div key={s.key} className="bg-paper-deep px-1.5 py-2 flex flex-col items-center gap-1 min-w-0">
                  ...
                  >
                    {s.opt.n.split(" ").slice(-1)[0]}
                  </Link>
                  <span className="text-[0.58rem] text-ink-muted tabular leading-none">{seasonLabel(s.opt.y)}</span>
                </div>
              ))}
            </div>
            ...
            {ROWS.map((row, ri) => {
              const { bestKey, worstKey } = rowExtremes(row);
              // See the note in the desktop table below: the caption marks the
              // start of a group, not every row that belongs to one.
              const opensSection = row.section && row.section !== ROWS[ri - 1]?.section;
              return (
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              bestKey === s.key && "bg-[color-mix(in_srgb,var(--good)_14%,transparent)] text-good font-semibold",
                          ...
                          >
                            {row.cellRender ? (
                              ...
                              <>
                                {row.format ? row.format(raw) : raw == null ? "—" : String(raw)}
                                {typeof pct === "number" && (
                                  <span
          ...
          <div className="hidden md:block">
            <table className="w-full text-sm">
              ...
              <tbody>
                {ROWS.map((row, ri) => {
                  const { bestKey, worstKey } = rowExtremes(row);
                  // The caption belongs to the GROUP, and `section` is set on every
                  // row in it, so this has to fire on the boundary rather than on
                  // the mere presence of a section — otherwise Ratings announces
                  // itself once for NET, again for ORTG and again for DRTG.
                  const opensSection = row.section && row.section !== ROWS[ri - 1]?.section;
                  return (
                    <Fragment key={row.key}>
                      {opensSection && (
        ...
        {buildingCard && (
          ...
          >
            <CompareShareCard
              ref={shareCardRef}
              kicker="Compare players"
              entities={filledSlots.map((s) => ({
                name: s.opt.n,
                sub: [seasonLabel(s.opt.y), s.opt.t].filter(Boolean).join(" · "),
                // `local` serves the same-origin crest — the GCS host sends no
                // CORS headers and would taint the canvas outright.
                logo: <TeamLogo name={s.opt.t} size={72} local eager />,
              }))}
              rows={buildShareRows()}
        ...
        {shareBlob && (
          <ShareScreenshotSheet
            blob={shareBlob}
            filename="bta-compare-players.png"
            caption={filledSlots.map((s) => s.opt.n).join(" vs ")}
...
<>
        ...
        <div
          className={cn(
            ...
            // and the right/left flipping below only ever chose which edge it
            // spilled over.
            "max-md:fixed max-md:inset-x-0 max-md:bottom-0 max-md:z-[60] max-md:rounded-t-xl",
            "max-md:pb-[env(safe-area-inset-bottom,0px)]",
            "md:absolute md:top-full md:mt-1 md:w-80 md:rounded-lg md:z-30",
            // Which edge it hangs from, at the widths where it is a dropdown.
            // Slot 3 is rightmost at every breakpoint; slot 1 only below lg.
            index === 3
              ? "md:right-0"
              : index === 1
                ? "md:right-0 lg:md:right-auto lg:left-0"
                : "md:left-0",
          )}
        >
