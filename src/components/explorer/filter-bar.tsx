    ...
    <ScopeCollapse summary={summary} pending={pending}>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          onClick={submit}
          disabled={!dirty}
          className="h-9 text-xs font-medium bg-coral text-white px-5 rounded-md hover:bg-coral-soft disabled:opacity-40 transition-colors"
        >
          Submit
        </button>
        <button type="button" onClick={reset} className="h-9 px-3 text-xs text-ink-muted hover:text-ink">Reset</button>
      </div>
    </ScopeCollapse>
  );
}
